                ...
                foreach(var _object in kilometer.Objects)
                {
                    ...
                    switch (_object)
                    {
                        ...
                        case UKSPS uksps:
                            ...
                            break;
                        case Cliffs cliffs:
                            objectImage = Resource1.arrow1;
                            break;
                        case LEP lep:
                            objectImage = Resource1.lep;
                            break;
                        case Crossing cross:
                            objectImage = Resource1.cross;
                            break;
                        case CrossingWithControll cross:
                            objectImage = Resource1.crossControl;
                            break;
                        default:
                            continue;
                    }
                    var state = graphics.Save();
                    graphics.TranslateTransform(point.X + size.Width / 2, currentY);
                    graphics.DrawImage(objectImage, new PointF(-(objectImage.Width / 2), offset));
                    graphics.Restore(state);
                    currentY += objectImage.Height + offset;
                }
